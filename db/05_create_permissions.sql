...
begin
    ...
    __can_read_global_journal = auth.has_permission(1, _user_id, 'system.journal.read_global_journal', false);
...
begin
    ...
    if (auth.is_owner(_target_user_id, _tenant_id)) then
        return true;
    end if;
...
as
$$
insert into auth.user_info(created_by, modified_by, can_login, email, display_name, username, original_username)
values ('initial_script', 'initial_script', false, 'system', 'System', 'system', 'system')
returning *;
...
begin
    ...
    perform auth.has_permission(_tenant_id, _user_id, 'system.groups.get_mappings');
...
create or replace function public.search_tenant_members(_requested_by text, _user_id bigint, _tenant_id int
, _search_text text
, _page int default 1
, _page_size int default 10)
--     maybe return less stuff
    returns table
            (
                __user_id           bigint,
                __user_display_name text,
...
declare
    __search_text text;
begin
    perform auth.has_permission(_tenant_id, _user_id, 'system.tenants.get_users');

    __search_text := helpers.unaccent_text(_search_text);

    _page := case when _page is null then 1 else _page end;
    _page_size := case when _page_size is null then 10 else _page_size end;

    return query with filtered_rows
                          as (select ui.user_id
                                   , count(1) over () as total_items
                              from auth.get_tenant_members(_requested_by, _user_id, _tenant_id) tm
                                       inner join user_info ui on tm.__user_id = ui.user_id
                              where (
                                                 helpers.unaccent_text(ui.email) like '%' || __search_text || '%'
                                            or  helpers.unaccent_text(ui.display_name) like '%' || __search_text || '%'
                                            or  helpers.unaccent_text(ui.username) like '%' || __search_text || '%'
                                        )
                              offset ((_page - 1) * _page_size) limit _page_size)
                 select ui.user_id,
                        ui.display_name as user_display_name,
                        ...
                        fr.total_items
                 from filtered_rows fr
                          inner join user_info ui on fr.user_id = ui.user_id
                 order by created desc;
end
$$;
...
begin
    ...
    if not auth.is_owner(_user_id, _tenant_id, _user_group_id)
        and not auth.is_owner(_user_id, _tenant_id)
    then
...
begin
    ...
    if
        ...
        insert into auth.user_info (created_by, modified_by, username, original_username, email, display_name,
                                    last_used_provider_code)
        values (_created_by, _created_by, __normalized_username, trim(_username), __normalized_email, _display_name,
                _last_provider_code)
...
begin
    ...
    perform unsecure.create_user_event(_modified_by, _user_id, 'password_changed',
...
create
    or replace function auth.load_permission_initial_data()
    returns setof int
    language plpgsql
as
$$
...
begin
    ...
    perform unsecure.create_permission_by_path_as_system('Get mappings', 'system.groups');
...
select *
from auth.load_permission_initial_data();
